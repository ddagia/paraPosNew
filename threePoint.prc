...
if (match(Cinternal(-k1,-k1-k2,m,m)));
  id k3 = 2*q-k1-k2;
  id Cinternal(-k1,-k1-k2,m,m) = Cinternal(-k1,-[k1+k2],m,m);
  id k2 = [k1+k2] - k1;
endif;
...
if (match(Cinternal(v1?$vec1,v2?$vec2,m?,m?)));
  ...
  multiply replace_(v1internal,$vec1,v2internal,$vec1 + $vec2);
  id v0prod(?a) = dd_(?a);
endif;
ModuleOption local, $vec1, $vec2;
#undefine cargs
